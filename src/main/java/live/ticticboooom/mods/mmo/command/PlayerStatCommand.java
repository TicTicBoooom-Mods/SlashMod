...
import live.ticticboooom.mods.mmo.cap.Capabilities;
...
public class PlayerStatCommand {
    public static void register(CommandDispatcher<CommandSource> dispatcher) {
        LiteralArgumentBuilder<CommandSource> cmd = Commands.literal("mmo_player_stat")
                .then(Commands.literal("overall").executes(x -> {
                    ...
                    player.sendMessage(new StringTextComponent(Float.toString(getPlayerProgress(x).getOverallLevel())), player.getUniqueID());
                    return 0;
                }));
    ...
    @SneakyThrows
    private static IPlayerAttributes getPlayerProgress(CommandContext<CommandSource> ctx){
        return ctx.getSource().asPlayer().getCapability(Capabilities.PLAYER_PROGRESS_CAPABILITY).orElse(null);
    }
